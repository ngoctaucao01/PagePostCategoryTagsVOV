<?php
/*
Plugin Name: Better Recent Posts Widget (with thumbnails)
...
class pippin_recent_posts extends WP_Widget {
	...
	function pippin_recent_posts() {
		...
		parent::__construct( false, $name = __( 'Better Recent Posts', 'better-recent-posts' ) );
	}
				...
				<li <?php if ( ! empty( $thumbnail_size ) ) {
					...
					<?php if ( $thumbnail == true ) { ?>
						<a href="<?php the_permalink(); ?>"
						   style="float: left; margin: 0 5px 0 0;"><?php the_post_thumbnail( array( $thumbnail_size ) ); ?></a>
					<?php } ?>
					<a href="<?php the_permalink(); ?>"><?php the_title(); ?></a><br/>
					<span class="time"><?php echo human_time_diff( get_the_time( 'U' ), current_time( 'timestamp' ) ) .
					                              ' ago'; ?></span>
				</li>
			<?php endforeach; ?>
			<?php $post = $tmp_post; ?>
	...
	function form( $instance ) {
		...
		echo self::get_feature_promo( "Lots more display options.", "http://zatzlabs.com/project/better-recent-posts-widget/" );
		?>
		<p>
		...
		<p>
			...
			<input class="widefat" id="<?php echo $this->get_field_id( 'cat' ); ?>"
			       name="<?php echo $this->get_field_name( 'cat' ); ?>" type="text" value="<?php echo $cat; ?>"/>
		</p>
		<p>
		...
		<p>
			<input id="<?php echo $this->get_field_id( 'thumbnail' ); ?>"
			       ...
			       value="1" <?php checked( '1', $thumbnail ); ?>/>
			<label for="<?php echo $this->get_field_id( 'thumbnail' ); ?>"><?php _e( 'Display thumbnails?' ); ?></label>
		</p>
		<p>
			<label
				for="<?php echo $this->get_field_id( 'thumbnail_size' ); ?>"><?php _e( 'Size of the thumbnails, e.g. <em>80</em> = 80px x 80px' ); ?></label>
			<input class="widefat" id="<?php echo $this->get_field_id( 'thumbnail_size' ); ?>"
			       name="<?php echo $this->get_field_name( 'thumbnail_size' ); ?>" type="text"
			       value="<?php echo $thumbnail_size; ?>"/>
		</p>
		<p>
			<label
				for="<?php echo $this->get_field_id( 'posttype' ); ?>"><?php _e( 'Choose the Post Type to display' ); ?></label>
			<select name="<?php echo $this->get_field_name( 'posttype' ); ?>"
			        id="<?php echo $this->get_field_id( 'posttype' ); ?>" class="widefat">
				<?php
				foreach ( $posttypes as $option ) {
					echo '<option value="' . $option->name . '" id="' . $option->name . '"', $posttype ==
					                                                                         $option->name ? ' selected="selected"' : '', '>', $option->name, '</option>';
				}
				?>
			</select>
		</p>
